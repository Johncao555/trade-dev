...
public class SeckillActivityServiceImpl implements SeckillActivityService {
     ...
     */
    @Override
    public Order processSeckillReqBase(long seckillActivityId, long userId) {
        ...
        if (!checkReslut) {
            throw new RuntimeException("stock deduct check in redis failed");
        }
        ...
        if (availableStock > 0) {
            ...
            // Add user to redis set as user who has purchased
            limitBuyService.addLimitMember(seckillActivityId, userId);
